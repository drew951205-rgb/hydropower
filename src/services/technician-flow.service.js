...
async function cancelByTechnician(user, event, orderId) {
  ...
  if (String(order.technician_id) !== String(user.id)) {
    await lineMessageService.replyText(event, '這張案件不是指派給你的案件，無法取消。');
  ...
  if (customer?.line_user_id) {
    await lineMessageService.pushMessages(customer.line_user_id, `師傅已取消案件 ${cancelled.order_no}，平台會協助後續處理。`);
  }

  await lineMessageService.replyText(event, '已取消案件，平台會協助後續處理。');
